  ...
  try {
    ...
    const createDb = async db => {
      ...
      if (!result.rows.length) {
        const existingUser = await client.query('SELECT 1 FROM pg_user WHERE usename = $1', [newUser])
    ...
    // Fail loudly. Previously this catch only logged and the process still
    // exited 0, so a failed CREATE DATABASE looked like success to the instant
    // tooling and hapi-fhir would then crash-loop on a missing `hapi` DB.
    // Setting a non-zero exit code makes the importer task report "Failed",
    // which aborts the deploy with a visible error instead of passing silently.
